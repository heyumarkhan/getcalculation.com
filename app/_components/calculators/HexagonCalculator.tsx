...
interface HexagonCalculatorProps {
  showTitle?: boolean;
  primaryColor?: string;
}
...
export default function HexagonCalculator({ 
  ...
  const calculate = () => {
    ...
    if (calculationType === 'area-from-side') {
      ...
      // Area = (3√3/2) × s²
      calculatedResult = (3 * Math.sqrt(3) / 2) * s * s;
      steps.push(`Given side length (s) = ${s}`);
      steps.push(`Area formula: A = (3√3/2) × s²`);
      steps.push(`A = (3 × √3 / 2) × ${s}²`);
      steps.push(`A = (3 × ${Math.sqrt(3).toFixed(6)} / 2) × ${s * s}`);
      steps.push(`A = ${(3 * Math.sqrt(3) / 2).toFixed(6)} × ${s * s}`);
      ...
      // Area = 2√3 × a²
      calculatedResult = 2 * Math.sqrt(3) * a * a;
      steps.push(`Given apothem (a) = ${a}`);
      steps.push(`Area formula: A = 2√3 × a²`);
      steps.push(`A = 2 × √3 × ${a}²`);
      steps.push(`A = 2 × ${Math.sqrt(3).toFixed(6)} × ${a * a}`);
      steps.push(`A = ${formatValue(calculatedResult, 6)} square units`);
    }
      ...
      calculatedResult = 6 * s;
      steps.push(`Given side length (s) = ${s}`);
      steps.push(`Perimeter formula: P = 6 × s`);
      steps.push(`P = 6 × ${s}`);
      ...
      // From A = (3√3/2) × s², we get s = √(2A / (3√3))
      calculatedResult = Math.sqrt(2 * a / (3 * Math.sqrt(3)));
      steps.push(`Given area (A) = ${a}`);
      steps.push(`From area formula: A = (3√3/2) × s²`);
      steps.push(`Solving for s: s = √(2A / (3√3))`);
      steps.push(`s = √(2 × ${a} / (3 × √3))`);
      steps.push(`s = √(${2 * a} / ${(3 * Math.sqrt(3)).toFixed(6)})`);
      steps.push(`s = √${(2 * a / (3 * Math.sqrt(3))).toFixed(6)}`);
      ...
      // Apothem = s × √3 / 2
      calculatedResult = s * Math.sqrt(3) / 2;
      steps.push(`Given side length (s) = ${s}`);
      steps.push(`Apothem formula: a = s × √3 / 2`);
      steps.push(`a = ${s} × √3 / 2`);
      steps.push(`a = ${s} × ${Math.sqrt(3).toFixed(6)} / 2`);
      ...
      calculatedResult = 2 * s;
      steps.push(`Given side length (s) = ${s}`);
      steps.push(`Long diagonal formula: d = 2 × s`);
      steps.push(`d = 2 × ${s}`);
  ...
  return (
    <>
      ...
      <Card className="max-w-md mx-auto">
        {showTitle && (
          <>
            <h2 className="text-2xl font-bold text-gray-900 mb-4">Hexagon Calculator</h2>
            <p className="text-gray-600 mb-6">Calculate area, perimeter, side length, apothem, and diagonal of a regular hexagon:</p>
          </>
        )}
