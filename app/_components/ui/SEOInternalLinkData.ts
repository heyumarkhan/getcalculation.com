// Server-safe data and functions for internal links
...
export const INTERNAL_LINKS = {
  ...
  'decimal-to-percent': { href: '/math/decimal-to-percent', title: 'Decimal to Percent Calculator' }
} as const;

// Server-safe helper function that returns HTML string (for use in dangerouslySetInnerHTML)
export function createInternalLinkHTML(key: keyof typeof INTERNAL_LINKS, text?: string): string {
  ...
  return `<a href="${link.href}" title="${link.title}" class="text-blue-600 hover:text-blue-800 underline">${escapedText}</a>`;
}
